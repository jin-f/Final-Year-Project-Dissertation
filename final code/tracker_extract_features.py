...
def extract_features(model, image_patch):
    # Resize the image patch to the required input size of the YOLOv5 model
    image_tensor = cv2.resize(image_patch, (640, 640))
    image_tensor = image_tensor.transpose(2, 0, 1)
    image_tensor = torch.from_numpy(image_tensor).unsqueeze(0).float()

    # Extract features using the YOLOv5 model
    features = model(image_tensor.cuda())

    # Remove the last dimension of the features tensor
    features = features[..., :2048]

    # Reshape the tensor to a 1D vector
...
def main():
    # initialise detector
    model = Detector(confidence_level=0.3, wanted_class='keyboard')
    ...
    while key != 113:
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            bbs = model.detect_tracking(image_np)

            features = []
            for detection in bbs:
                box, confidence, detection_class = detection
                x1 = box[0]
                y1 = box[1]
                x2 = box[2] + x1
                y2 = box[3] + y1

                image_patch = image_np[int(y1):int(y2), int(x1):int(x2)]
                feature_model = model.get_model()
                feature = extract_features(feature_model, image_patch)
                features.append(feature)
        
            # object tracking
            tracks = object_tracker.update_tracks(bbs, embeds=features)
            for track in tracks:
